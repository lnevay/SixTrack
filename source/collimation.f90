  ...
  if(firstrun .and. iturn == 1) then
    ...
    if (isACrystal) then
      ...
      if(modulo(cdb_cRotation(icoll),pi) < c1m9) then
        crystalAngle = -(sqrt(emitX/tbetax(ie))*talphax(ie))*cdb_cNSig(icoll) +  cdb_cryTilt(icoll)
      elseif (modulo(cdb_cRotation(icoll)-pi2,pi) < c1m9) then
        crystalAngle = -(sqrt(emitY/tbetay(ie))*talphay(ie))*cdb_cNSig(icoll) +  cdb_cryTilt(icoll)
      else
